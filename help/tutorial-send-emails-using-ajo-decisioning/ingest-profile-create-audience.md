---
title: 프로필 수집 및 대상자 만들기
description: 스키마 만들기, 프로필 수집 및 대상 만들기
feature: Decisioning
role: User
level: Beginner
doc-type: Tutorial
last-substantial-update: 2025-10-31T00:00:00Z
jira: KT-19558
source-git-commit: b70717bd66deb3c070c6dd155f13f8cb72fce7c3
workflow-type: tm+mt
source-wordcount: '79'
ht-degree: 0%

---

# 프로필 수집 및 대상자 만들기

이 비디오에서는 Adobe Experience Platform에서 프로필 스키마를 만들고, CSV 파일에서 고객 데이터를 수집하고, 프로필 속성을 사용하여 대상을 구축하는 방법을 알아봅니다.

이름, 이메일, 충성도 상태 및 연간 수입과 같은 주요 필드를 정의하고 실제 고객 데이터를 업로드한 다음 해당 데이터를 사용하여 가치가 높은 고객의 대상을 만드는 방법을 알아봅니다.

>[!VIDEO](https://video.tv.adobe.com/v/3476445/?captions=kor&learn=on)