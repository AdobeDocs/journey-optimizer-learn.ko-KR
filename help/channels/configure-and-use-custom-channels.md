---
title: 사용자 정의 채널 구성 및 사용
description: '사용자 정의 채널을 사용하여 Adobe Journey Optimizer에서 기본적으로 지원하지 않는 메시징 채널(예: Slack, WeChat, Telegram 또는 자체 플랫폼)을 연결하는 방법을 알아보세요.'
feature: Channels
role: Admin, User
level: Beginner, Intermediate
doc-type: Feature Video
duration: 417
last-substantial-update: '2026-07-24T00:00:00.000Z'
jira: KT-22143
badgeLA: label="제한된 가용성" type="Informative"
autotag-review: '2026-07-24T14:40:32.756Z'
TQID: 'https://experienceleague.adobe.com/j5VStzJxi1YMt9-CGmfjt0WKaUDw-sIRN7qScsExcDk'
product_v2:
  - id: cb954087-f4fc-4456-afb9-e939cabcdc79
feature_v2:
  - id: d0a62d3c-b79e-47e4-929e-40ef3cffa037
source-git-commit: 507316431c32151c0b07499c60a261fed57f391e
workflow-type: ht
source-wordcount: 80
ht-degree: 100%

---


# 사용자 정의 채널 구성 및 사용

{{limited-availability}}

사용자 정의 채널을 사용하여 Adobe Journey Optimizer에서 기본적으로 지원하지 않는 메시징 채널(예: Slack, WeChat, Telegram 또는 자체 플랫폼)을 연결하는 방법을 알아보세요.

이 비디오에서는 사용자 정의 아웃바운드 채널을 처음부터 끝까지 구축하고 여정에서 사용하는 방법을 안내합니다.

>[!VIDEO](https://video.tv.adobe.com/v/3496291/?captions=kor&learn=on&enablevpops)
