---
user-guide-title: Journey Optimizer에서 브랜드 내 컨텐츠 만들기
user-guide-description: 이 튜토리얼에서는 브랜드 지침을 작성 및 구성하는 것에서부터 전체 캠페인 생성, 개별 콘텐츠 구성 요소 편집, 시각적 요소 향상 및 다국어 경험 활성화에 이르기까지 모든 과정을 안내합니다. 또한 모든 커뮤니케이션에서 자동화된 품질 검사를 적용하고 브랜드 정렬을 보장하며 거버넌스를 유지하는 방법을 살펴볼 수 있습니다. 결과적으로 전체 채널에 걸쳐 콘텐츠 생성 작업을 능률화하고, 일관성을 향상시키고, 고품질의 개인화된 경험을 제공할 수 있습니다.
role: User
level: Internediate
breadcrumb-title: brand-on-content 만들기
breadcrumb-url: /docs/journey-optimizer-learn/reate-on-brand-content/introduction
auto-video-transcripts: true
source-git-commit: 8a5f2a79a6f7ad8e0d42d5d148ea5a2434e67b26
workflow-type: tm+mt
source-wordcount: '122'
ht-degree: 0%

---


# Journey Optimizer에서 브랜드 내 컨텐츠 만들기 {#create-on-brand-content}

+ 소개 및 브랜드 설정 {#introduction-and-brand-setup}
   + [소개](./introduction.md)
   + [브랜드 구성](./configure-brands.md)
   + [브랜드 지침 팁](./brands-tips-and-tricks.md)
+ 콘텐츠 생성 및 세분화 {#generate-content}
   + [처음부터 콘텐츠 만들기](./create-content-from-scratch.md)
   + [섹션 콘텐츠 편집 및 개인화](./edit-and-personalize-section-content.md)
   + [시각적 콘텐츠 향상](./enhance-your-visual-content.md)
   + [콘텐츠의 다국어 버전 만들기 및 세분화](./create-and-refine-multilingual-content.md)
+ [품질, 브랜드 정렬 및 거버넌스 보장](./quality-brand-alignment-and-governance.md)



