---
title: 처음부터 콘텐츠 만들기
description: 이 비디오에서는 템플릿에서 시작하고, 브랜드 지침을 적용하고, 간단한 프롬프트를 사용하여 이메일, 푸시 알림 및 인앱 메시지를 처음부터 만드는 방법을 보여 줍니다. 또한 AI 기반의 변형 및 점수를 활용하여 옴니채널 경험에서 고품질의 일관된 콘텐츠를 빠르게 생성하는 동시에 톤, 이미지 및 메시징을 개선하는 방법도 살펴볼 수 있습니다.
feature: Content Assistant
topic: Content Management
role: User
level: Intermediate
doc-type: Feature Video
duration: 182
last-substantial-update: 2026-07-09T00:00:00Z
jira: KT-16226
source-git-commit: 935e14c72ee3c912fb0071c9eaee03f3cfb2e597
workflow-type: tm+mt
source-wordcount: '119'
ht-degree: 0%

---


# 처음부터 콘텐츠 만들기

이 비디오에서는 템플릿에서 시작하고, 브랜드 지침을 적용하고, 간단한 프롬프트를 사용하여 이메일, 푸시 알림 및 인앱 메시지를 처음부터 만드는 방법을 보여 줍니다. AI 기반 변형 및 점수를 사용하여 톤, 이미지 및 메시지를 구체화하는 방법도 살펴볼 수 있습니다. 이렇게 하면 옴니채널 경험 전반에 걸쳐 고품질의 일관된 콘텐츠를 신속하게 만들 수 있습니다.

>[!VIDEO](https://video.tv.adobe.com/v/3434642/?captions=kor&learn=on&enablevpops)
