...
> - [Adobe Journey Optimizer에서 지능형 여정 활성화를 위한 데이터 엔지니어](https://experienceleague.adobe.com/en/courses/ajo-engineer-data-for-intelligent-journey-activation)
>*이 교육 과정에서는 Experience Platform을 사용하여 Journey Optimizer에 대한 실시간 고객 프로필 데이터를 설정하고 관리하는 방법에 대해 알아봅니다. 데이터 모델링, ID 매핑 및 데이터 수집을 이해하여 개인화된 고객 여정을 위한 통합 프로필을 만듭니다.*
...
Journey Optimizer의 모바일 구현은 앱의 Adobe Experience Platform Mobile SDK **통합으로**&#x200B;시작됩니다. SDK는 데이터 수집과 Adobe Experience Platform(AEP) 및 Adobe Journey Optimizer(AJO)과 같은 해당 애플리케이션과의 상호 작용에 필수적입니다.

Mobile SDK는

- 앱 이벤트(화면 보기, 탭, 구매, 라이프사이클 이벤트 등)를 수집하여 Adobe Experience Platform 에지 네트워크&#x200B;**로**&#x200B;보냅니다.
...
- **Journey Optimizer 모바일 확장 기능**(푸시, 인앱, 콘텐츠 카드, 의사 결정)과 통합되므로 메시지를 끝까지 전달, 렌더링 및 측정할 수 있습니다.

모바일 SDK을 앱에 통합하지 않으면 Journey Optimizer에서 안정적으로 다음을 수행할 수 없습니다.

- 모바일 푸시 및 인앱 메시지를 전달하고 추적합니다.
- 컨텐츠 카드를 렌더링하고 추적합니다.
- 실시간 인앱 동작을 사용하여 여정을 트리거하고 경험을 개인화할 수 있습니다.
...
- 조직에 프로비저닝된 Adobe Systems AJO(Journey Optimizer)입니다.
- 데이터 수집 및 Journey Optimizer 권한이 있는 Adobe Experience Platform 액세스
...
### Journey Optimizer에 필요한 모바일 SDK 구성 요소

Journey Optimizer 모바일 채널(푸시, 인앱, 콘텐츠 카드, 코드 기반 경험)을 사용하려면 모바일 앱에서 **코어** 및 **채널별** 확장 집합을 설치하고 구성해야 합니다.

>[!BEGINTABS]

>[!TAB 코어]

#### 코어 확장 (모든 모바일 사용 사례에 필요)

| 용도 | 확장 예(iOS/Android) | 참고 |
|----------------------|-----------------------------------------------|-------|
| 이벤트 허브 및 서비스 | 모바일 코어/AEP 코어, AEP 서비스 | 다른 모든 확장의 경우 기초입니다. 이벤트 허브, 네트워킹, 스토리지 및 공유 상태를 제공합니다. |
| 에지 네트워크 | Adobe Experience Platform Edge Network | Adobe Experience Platform 에지 네트워크에 앱 이벤트를 보냅니다. |
| ID | 에지 네트워크의 ID | 프로필 및 세분화에 사용되는 ECID 및 기타 ID를 관리합니다. |
| 동의 | 에지 네트워크에 대한 동의 | 사용자 동의 기본 설정을 수집하고 적용합니다. |
| Assurance | Adobe Experience Platform Assurance | 종단 간 SDK 및 채널 구성의 유효성을 검사하고 디버그 하는 데 사용됩니다. |

>[!TAB 채널별]

#### Journey Optimizer용 채널별 확장

| 채널/기능 | 추가 키 확장(코어 위에) | 지원 사항 |
|------------------------|---------------------------------------------------------------------|------------------|
| 푸시 알림 | Journey Optimizer 모바일 확장(푸시) | 푸시 토큰 등록 및 업데이트, 푸시 추적 이벤트 전송, AJO 푸시 구성에 연결. |
| 인앱 메시징 | Journey Optimizer 모바일 확장 프로그램(인앱), 메시징 UI 구성 요소 | 인앱 메시지를 가져와서 컨텍스트에 표시하고 노출 횟수 및 상호 작용 이벤트를 보냅니다. |
| 콘텐츠 카드 | 컨텐츠 카드 지원을 통해 SDK에 메시지 보내기 | 정확한 Journey Optimizer 보고를 위해 콘텐츠 카드를 가져오고, 렌더링하고, 추적합니다. |
| 코드 기반 경험 | Journey Optimizer/decisioning 확장 프로그램 또는 Edge Server API | 앱의 특정 &quot;표면&quot;에 대한 결정을 가져옵니다. 앱은 콘텐츠가 렌더링되는 방식을 제어합니다. |

>[!ENDTABS]

#### 모바일 태그 속성 및 구성

AEP 데이터 수집(태그)의 **[모바일 태그 속성](https://developer.adobe.com/client-sdks/home/getting-started/create-a-mobile-property/)**&#x200B;에서 이러한 확장을 구성합니다. 이 속성은 다음을 제어합니다.

- 설치된 Mobile SDK 확장.
- Edge Network에 대한 호출을 트리거하는 앱의 이벤트입니다.
- 데이터가 XDM에 매핑되고 Adobe Systems 솔루션(Journey Optimizer, Analytics 등)에 전달되는 방법.

이 모바일 속성 수동으로 생성 및 구성하거나 Guided Channel Setup **[](https://experienceleague.adobe.com/en/docs/journey-optimizer-learn/tutorials/configuration/channel-configuration/web-and-mobile-channels/guided-channel-setup)을 사용하여** iOS 또는 Android에 필요한 태그 속성, 데이터 스트림 및 채널 구성을 자동으로 생성할 수 있습니다.
...
<div class="columns">
            ...
            <div class="card-image">
                <figure class="image x-is-16by9">
                    <a href="https://experienceleague.adobe.com/en/docs/platform-learn/data-collection/mobile-sdk/overview" title="Adobe Experience Platform Mobile SDK 개요" target="_blank" rel="referrer">
                        <img class="is-bordered-r-small" src="https://video.tv.adobe.com/v/28948?format=jpeg&nocache=1763594622398" alt="Adobe Experience Platform Mobile SDK 개요"
    ...
    <div class="column is-half-tablet is-half-desktop is-one-third-widescreen" aria-label="Adobe Experience Platform Mobile SDK Documentation">
        <div class="card" style="height: 100%; display: flex; flex-direction: column; height: 100%;">
            <div class="card-image">
                <figure class="image x-is-16by9">
                    <a href="https://experienceleague.adobe.com/en/docs/mobile" title="Adobe Experience Platform Mobile SDK 설명서" target="_blank" rel="referrer">
                        <img class="is-bordered-r-small" src="https://cdn.experienceleague.adobe.com/thumb/exl-cards/documentation.png" alt="Adobe Experience Platform Mobile SDK 설명서"
                             style="width: 100%; aspect-ratio: 16 / 9; object-fit: cover; overflow: hidden; display: block; margin: auto;">
                    </a>
                </figure>
            </div>
            <div class="card-content is-padded-small" style="display: flex; flex-direction: column; flex-grow: 1; justify-content: space-between;">
                <div class="top-card-content">
                    <p class="headline is-size-6 has-text-weight-bold">
                        <a href="https://experienceleague.adobe.com/en/docs/mobile" target="_blank" rel="referrer" title="Adobe Experience Platform Mobile SDK 설명서">Adobe Experience Platform Mobile SDK 설명서</a>
                    </p>
                    <p class="is-size-6">Experience Platform Mobile SDK에서 자체 도움말 문서 및 튜토리얼을 검색합니다. 라이브 및 온디맨드 비디오 이벤트에서 전문가로부터 전략과 모범 사례를 알아보십시오.</p>
                </div>
...
자세한 내용은 [Adobe Experience Platform Mobile SDK 설명서](https://experienceleague.adobe.com/en/docs/mobile)

#### 개발자 참조:

- [Mobile SDK 개발자 포털(홈)](https://developer.adobe.com/client-sdks/home/)
- [현재 SDK 버전](https://developer.adobe.com/client-sdks/home/current-sdk-versions/)
- [모바일 속성(태그) 시작](https://developer.adobe.com/client-sdks/home/getting-started/create-a-mobile-property/)
- [SDK 가져오기(앱에 설치)](https://developer.adobe.com/client-sdks/home/getting-started/get-the-sdk/)
- [Mobile SDK으로 이벤트 추적](https://developer.adobe.com/client-sdks/home/getting-started/track-events/)
- [확실한 검증](https://developer.adobe.com/client-sdks/home/base/assurance/)

#### 모바일 SDK 준비 상태 체크리스트

> - [ ] Core SDK가 설치됨(Core, Edge, ID, Consent, Assurance).
> - [ ] 사용할 채널(푸시, 인앱, 컨텐츠 카드, 코드 기반)에 대해 Journey Optimizer 모바일 확장이 추가되었습니다.
> - [ ] 데이터 스트림이 이벤트 및 프로필 데이터 세트로 올바르게 구성되었습니다.
> - [ ] ID 및 동의는 Assurance를 통해 구현되고 검증됩니다.
        ...
        <div class="card" style="height: 100%; display: flex; flex-direction: column; height: 100%;">
            <div class="card-image">
                <figure class="image x-is-16by9">
                    <a href="https://experienceleague.adobe.com/en/docs/journey-optimizer-learn/tutorials/configuration/channel-configuration/web-and-mobile-channels/guided-channel-setup" title="가이드 채널 설정" target="_blank" rel="referrer">
                        <img class="is-bordered-r-small" src="https://video.tv.adobe.com/v/3433053/?format=jpeg&nocache=1763594622823" alt="가이드 채널 설정"
                             style="width: 100%; aspect-ratio: 16 / 9; object-fit: cover; overflow: hidden; display: block; margin: auto;">
                    </a>
                </figure>
            </div>
            <div class="card-content is-padded-small" style="display: flex; flex-direction: column; flex-grow: 1; justify-content: space-between;">
                <div class="top-card-content">
                    ...
                    <p class="is-size-6">Experience Platform, Journey Optimizer 및 데이터 수집에서 웹 및 모바일 채널을 신속하게 설정 및 확인하고 샘플 iOS 마케팅 앱에 대한 푸시 알림을 구성하는 방법을 알아봅니다.</p>
                </div>
            ...
            <div class="card-image">
                <figure class="image x-is-16by9">
                    <a href="https://experienceleague.adobe.com/ko/docs/journey-optimizer-learn/tutorials/configuration/channel-configuration/whatsapp-channel/set-up-whatsapp-channel" title="WhatsApp 채널 설정" target="_blank" rel="referrer">
                        <img class="is-bordered-r-small" src="https://video.tv.adobe.com/v/3470268/?format=jpeg&nocache=1763594622814" alt="WhatsApp 채널 설정"
                             style="width: 100%; aspect-ratio: 16 / 9; object-fit: cover; overflow: hidden; display: block; margin: auto;">
                    </a>
                </figure>
            </div>
            ...
            <div class="card-image">
                <figure class="image x-is-16by9">
                    <a href="https://experienceleague.adobe.com/en/docs/journey-optimizer-learn/tutorials/configuration/channel-configuration/sms-mms-channel/set-up-sms-channel" title="SMS API 자격 증명 및 채널 표면 구성" target="_blank" rel="referrer">
                        <img class="is-bordered-r-small" src="https://video.tv.adobe.com/v/3413355?format=jpeg&nocache=1763594624036" alt="SMS API 자격 증명 및 채널 표면 구성"
                             style="width: 100%; aspect-ratio: 16 / 9; object-fit: cover; overflow: hidden; display: block; margin: auto;">
                    </a>
                </figure>
            </div>
        ...
        <div class="card" style="height: 100%; display: flex; flex-direction: column; height: 100%;">
            <div class="card-image">
                <figure class="image x-is-16by9">
                    <a href="https://experienceleague.adobe.com/ko/docs/journey-optimizer-learn/tutorials/configuration/channel-configuration/sms-mms-channel/configure-custom-sms-provider" title="사용자 정의 SMS 공급자 구성" target="_blank" rel="referrer">
                        <img class="is-bordered-r-small" src="https://video.tv.adobe.com/v/3431625/?format=jpeg&nocache=1763594624067" alt="사용자 정의 SMS 공급자 구성"
                             style="width: 100%; aspect-ratio: 16 / 9; object-fit: cover; overflow: hidden; display: block; margin: auto;">
                    </a>
                </figure>
            </div>
            <div class="card-content is-padded-small" style="display: flex; flex-direction: column; flex-grow: 1; justify-content: space-between;">
                <div class="top-card-content">
                    ...
                    <p class="is-size-6">Journey Optimizer에서 사용자 지정 SMS 공급자를 구성하고, API 자격 증명 및 웹훅을 설정하고, 옵트인/옵트아웃 키워드를 관리하고, 개인화된 캠페인을 실행 방법을 알아봅니다.</p>
                </div>
            ...
            <div class="card-image">
                <figure class="image x-is-16by9">
                    <a href="https://experienceleague.adobe.com/en/docs/journey-optimizer-learn/tutorials/configuration/channel-configuration/sms-mms-channel/configure-mms-api-credentials-and-channel-surfaces" title="MMS API 자격 증명 및 채널 표면 구성" target="_blank" rel="referrer">
                        <img class="is-bordered-r-small" src="https://video.tv.adobe.com/v/3428872/?format=jpeg&nocache=1763594624083" alt="MMS API 자격 증명 및 채널 표면 구성"
                             style="width: 100%; aspect-ratio: 16 / 9; object-fit: cover; overflow: hidden; display: block; margin: auto;">
                    </a>
                </figure>
            </div>
            ...
            <div class="card-image">
                <figure class="image x-is-16by9">
                    <a href="https://experienceleague.adobe.com/ko/docs/journey-optimizer-learn/tutorials/configuration/channel-configuration/sms-mms-channel/set-up-rcs" title="Journey Optimizer에서 RCS 설정" target="_blank" rel="referrer">
                        <img class="is-bordered-r-small" src="https://video.tv.adobe.com/v/3464755/?format=jpeg&nocache=1763594624043" alt="Journey Optimizer에서 RCS 설정"
                             style="width: 100%; aspect-ratio: 16 / 9; object-fit: cover; overflow: hidden; display: block; margin: auto;">
                    </a>
                </figure>
            </div>
...
- [더 빠른 개인화를 위해 모바일에서 CDN 기반 ODD(클라이언트 측 개인화)를 사용합니다.](https://experienceleaguecommunities.adobe.com/t5/journey-optimizer-blogs/using-cdn-based-client-side-personalization-odd-on-mobile-for/ba-p/761626)
- [Adobe Experience Cloud 용 Mobile Activation](https://experienceleaguecommunities.adobe.com/t5/adobe-target-blogs/mobile-activation-for-adobe-experience-cloud/ba-p/541595)

## 개인 정보 보호법 및 플랫폼 지침을 준수하는지 확인합니다.
    ...
    <div class="column is-half-tablet is-half-desktop is-one-third-widescreen" aria-label="Privacy Features in Adobe Journey Optimizer">
        <div class="card" style="height: 100%; display: flex; flex-direction: column; height: 100%;">
            ...
            <div class="card-content is-padded-small" style="display: flex; flex-direction: column; flex-grow: 1; justify-content: space-between;">
                <div class="top-card-content">
                    <p class="headline is-size-6 has-text-weight-bold">
                        <a href="https://experienceleague.adobe.com/en/docs/journey-optimizer/using/privacy/privacy-landing-page" target="_blank" rel="referrer" title="Adobe Journey Optimizer의 개인 정보 기능">Adobe Systems Journey Optimizer의 개인 정보 보호 기능</a>
                    </p>
                    <p class="is-size-6">개인 정보 보호 요청을 처리하고, 사용자 작업을 감사하고, 동의를 관리하고, 거버넌스 규칙을 적용하고, 고객 관리형 키좋아요 고급 보안 옵션을 활용 방법을 알아봅니다.</p>
                </div>
            ...
            <div class="card-image">
                <figure class="image x-is-16by9">
                    <a href="https://experienceleague.adobe.com/ko/docs/journey-optimizer-learn/tutorials/data-governance-and-privacy/data-governance-framework" title="데이터 거버넌스 프레임워크 개요" target="_blank" rel="referrer">
                        <img class="is-bordered-r-small" src="https://video.tv.adobe.com/v/29708/?format=jpeg&nocache=1763594624934" alt="데이터 거버넌스 프레임워크 개요"
                             style="width: 100%; aspect-ratio: 16 / 9; object-fit: cover; overflow: hidden; display: block; margin: auto;">
                    </a>
                </figure>
            </div>
            ...
            <div class="card-image">
                <figure class="image x-is-16by9">
                    <a href="https://experienceleague.adobe.com/ko/docs/journey-optimizer-learn/tutorials/data-governance-and-privacy/classify-data-using-lables" title="레이블을 사용한 데이터 분류" target="_blank" rel="referrer">
                        <img class="is-bordered-r-small" src="https://video.tv.adobe.com/v/29709?format=jpeg&nocache=1763594624932" alt="레이블을 사용한 데이터 분류"
                             style="width: 100%; aspect-ratio: 16 / 9; object-fit: cover; overflow: hidden; display: block; margin: auto;">
                    </a>
                </figure>
            </div>
            ...
            <div class="card-image">
                <figure class="image x-is-16by9">
                    <a href="https://experienceleague.adobe.com/ko/docs/journey-optimizer-learn/tutorials/data-governance-and-privacy/create-data-usage-policies" title="데이터 사용 정책 만들기" target="_blank" rel="referrer">
                        <img class="is-bordered-r-small" src="https://video.tv.adobe.com/v/32977/?format=jpeg&nocache=1763594624933" alt="데이터 사용 정책 만들기"
                             style="width: 100%; aspect-ratio: 16 / 9; object-fit: cover; overflow: hidden; display: block; margin: auto;">
                    </a>
                </figure>
            </div>
...
대부분의 모바일 문제는 Journey Optimizer 여정 또는 캠페인 자체가 아니라 SDK 또는 데이터 수집 구성&#x200B;**에서 발생합니다**. 아래 표를 사용하여 무엇이 잘못되었는지 확인한 다음, 자세한 내용을 보려면 해당 섹션을 확장하십시오.
...
| 2 | 배치 추적 실패, AEP 오류 | [데이터 스트림 또는 데이터 세트가 잘못 구성됨](#2-misconfigured-datastreams-or-datasets) | 이벤트를 이벤트 데이터 세트에 매핑하고 프로필을 프로필 데이터 세트에 매핑 |
| 3 | 여정은 발사되지 않습니다. 이상한 개인화 | [ID 또는 동의 누락/불일치](#3-missing-or-inconsistent-identity-and-consent) | Edge ID 및 동의 구현; 보증에서 확인 |
| 4 | 푸시 배달 또는 보고서에서 열기 없음 | [푸시 토큰 등록 또는 추적이 끊어짐](#4-push-token-registration-and-tracking-not-wired-correctly) | SDK를 통한 토큰 등록 및 상호 작용 추적 수정 |
| 5 | 활성 캠페인에도 불구하고 인앱 노출 없음 | [인앱 메시지 또는 컨텐츠 카드가 표시되지 않음](#5-in-app-messages-or-content-cards-not-displaying) | 메시지 확장, 트리거 및 보증 결정 응답 확인 |

### 함정에 대한 자세한 지침

증상과 일치하는 함정을 열어 확인해야 할 사항과 해결 방법을 확인하십시오.

<details id="1-sdk-versions-and-extensions-not-aligned-with-channel-requirements">
<summary><strong>1. 채널 요구사항에 부합하지 않는 SDK 버전 및 확장 프로그램</strong></summary>

**표시되는 내용**

- 푸시 또는 인앱 활동이 디바이스 내에 도달하지 않습니다.
- 안내식 채널 설정 또는 채널 유효성 검사 실패.
...
참조: [모바일 및 웹 설정](https://experienceleague.adobe.com/ko/docs/journey-optimizer/using/configuration/guided-setup/set-mobile-config)

</details>


<details id="2-misconfigured-datastreams-or-datasets">
<summary><strong>2. 잘못 구성된 데이터 스트림 또는 데이터 세트</strong></summary>
...
참조: [Adobe Journey Optimizer의 푸시 알림 흐름](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/channels/push/push-config/push-gs)

</details>


<details id="3-missing-or-inconsistent-identity-and-consent">
<summary><strong>3. ID 및 동의가 누락되었거나 일치하지 않음</strong></summary>
...
- ID 및 동의가 모든 관련 이벤트에 표시될 때까지 보증에서 다시 테스트합니다.

참조: [Platform Mobile SDK 구현에 대한 동의 구현](https://experienceleague.adobe.com/en/docs/platform-learn/implement-mobile-sdk/app-implementation/consent)

</details>


<details id="4-push-token-registration-and-tracking-not-wired-correctly">
<summary><strong>4. 푸시 토큰 등록 및 추적이 올바르게 연결되지 않음</strong></summary>

**표시되는 내용**

- 캠페인 또는 여정이 실행되더라도 사용자는 푸시 알림을 균일 수신하지 않습니다.
- 푸시 보고서에는 열기, 닫기 또는 상호 작용이 표시되지 않습니다.

**확인 사항**

- 앱이 푸시 토큰을 Journey Optimizer 확장에 등록합니까?
   - 처음 설치할 때?
   - 앱을 업데이트할 때마다?
   - OS가 토큰을 새로 고칠 때마다?
- 사용자 사용자가 알림 메시지를 열거나 닫을 때 Assurance에 추적 이벤트가 표시되나요?

**해결 방법**

- 다음과 같은 코드를 추가하거나 수정합니다.
   - 토큰을 만들거나 새로 고칠 때마다 Journey Optimizer 모바일 확장을 통해 토큰을 등록합니다.
   - Mobile SDK를 통해 푸시 상호 작용 이벤트(열기, 닫기, 사용자 지정 작업)를 보냅니다.
- 보증을 사용하여 등록 및 추적 이벤트가 예상대로 실행되는지 확인합니다.

참조: [Adobe Systems Journey Optimizer의 푸시 알림 흐름](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/channels/push/push-config/push-gs)

</details>


<details id="5-in-app-messages-or-content-cards-not-displaying">
<summary><strong>5. 인앱 메시지 또는 컨텐츠 카드가 표시되지 않음</strong></summary>
...
- 필요한 메시지 확장을 설치하고 등록합니다.
- 타겟 이벤트(화면, 사용자 지정 이벤트)에 대한 결정을 트리거하는 규칙을 추가하거나 수정합니다.
- 컨텐츠 카드의 경우 다음 사항을 확인합니다.
   - Messaging SDK API를 통해 카드를 가져옵니다.
   - UI 내에서 렌더링합니다.
   - SDK를 통해 상호 작용을 다시 추적합니다.

다음을 참조하십시오.
- [인앱 메시지 만들기 및 보내기](https://experienceleague.adobe.com/en/docs/platform-learn/implement-mobile-sdk/experience-cloud/journey-optimizer/journey-optimizer-inapp)
- [Mobile SDK에서 컨텐츠 카드 지원 구성](https://experienceleague.adobe.com/en/docs/journey-optimizer/using/channels/content-card/configure/content-card-lp)
...
앱을 마케터에게 전달하기 전에 **[Assurance](https://developer.adobe.com/client-sdks/home/base/assurance/)**&#x200B;에서 다음을 확인하십시오.

- 코어 SDK + Journey Optimizer 확장이 로드됨,
- 이벤트가 올바른 데이터 스트림 및 데이터 세트에서 흐르고 있습니다.
- ID 및 동의는 모든 주요 이벤트에 존재합니다.
- 푸시 토큰 및 상호 작용이 추적되고
- 하나 이상의 테스트 인앱 메시지 또는 콘텐츠 카드가 다음으로 표시 및 기록됨