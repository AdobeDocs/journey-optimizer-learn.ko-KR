---
title: 여정 노드를 조각으로 저장
description: Adobe Journey Optimizer에서 여정 노드를 재사용 가능한 조각으로 저장하는 방법에 대해 알아봅니다. 워크플로우를 간소화하고 여러 여정 간에 일관성을 보장합니다.
feature: Journeys
role: User
level: Beginner
doc-type: Feature Video
duration: 11
last-substantial-update: '2026-05-20T00:00:00.000Z'
jira: KT-21345
autotag-review: '2026-05-20T23:14:06.458Z'
TQID: 'https://experienceleague.adobe.com/FB2ifhGA6-XNe3AGbDCor7wbfw8TgAJp6xHm-WcGiBQ'
product_v2:
  - id: cb954087-f4fc-4456-afb9-e939cabcdc79
feature_v2:
  - id: d998adac-2f81-400b-a669-d07bb196e4eb
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2:
  - id: e8ccd51f-da0d-4e3b-939b-e30d5ebb1ea5
source-git-commit: 3defd5900b460509d7dd859aab09d32778db5551
workflow-type: tm+mt
source-wordcount: 84
ht-degree: 0%

---


# 여정 노드를 조각으로 저장

Adobe Journey Optimizer에서 재사용 가능한 여정 조각을 만드는 방법을 알아봅니다. 이 기능을 사용하면 여정 노드 집합을 조각으로 저장하여 다른 여정에서 빠르게 재사용할 수 있습니다. 조각을 사용하면 시간을 절약하고 워크플로를 간소화할 수 있으며 여러 여정 간에 일관성을 보장할 수 있습니다. 포함된 비디오를 통해 자세한 연습 및 실제 예제를 살펴보십시오.

>[!VIDEO](https://video.tv.adobe.com/v/3491394/?captions=kor&learn=on&enablevpops)

