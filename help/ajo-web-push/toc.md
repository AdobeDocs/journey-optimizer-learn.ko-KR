---
title: Adobe Journey Optimizer의 웹 푸시
description: Adobe Journey Optimizer에서 웹 푸시 메시지를 보내는 방법 알아보기
user-guide-title: Adobe Journey Optimizer에서 캠페인 및 여정을 사용하여 푸시 메시지 보내기
user-guide-description: 이 튜토리얼에서는 캠페인 및 여정의 을 사용하여 Adobe Journey Optimizer에서 웹 푸시 메시지를 전송하는 데 필요한 단계를 안내합니다
solution: Journey Optimizer
role: Developer
level: Beginner
breadcrumb-url: /docs/journey-optimizer-learn/ajo-web-push/introduction
source-git-commit: c339fe796af1e691cd3b1c98cd6ba8a8772551e4
workflow-type: tm+mt
source-wordcount: '76'
ht-degree: 7%

---


# Adobe Journey Optimizer의 웹 푸시{#ajo-web-push}

+ [소개](./introduction.md)
+ [푸시 구성 만들기](./create-push-configuration.md)
+ [데이터스트림 만들기](./create-data-stream.md)
+ [로컬 설정](./run-application-locally.md)
+ [사용자 권한 캡처](./capture-user-persmission.md)
+ [대상자 만들기](./create-audience.md)
+ [캠페인 만들기](./create-campaign.md)
+ [태그 속성 만들기](./create-tag-property.md)
+ [여정 만들기](./create-journey.md)
+ [디버깅](./debugging.md)

