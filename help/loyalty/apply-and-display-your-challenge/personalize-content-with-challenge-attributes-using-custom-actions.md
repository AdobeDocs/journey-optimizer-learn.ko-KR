---
title: 사용자 지정 작업을 사용하여 과제 속성으로 콘텐츠 개인화
description: 사용자 지정 작업으로 과제 데이터를 검색하고 이를 사용하여 각 구성원의 과제 진행 상황 및 보상에 따라 콘텐츠 카드 및 코드 기반 경험을 동적으로 개인화하는 방법에 대해 알아봅니다.
role: User
level: Experienced
doc-type: Feature Video
duration: 475
last-substantial-update: '2026-09-11T00:00:00.000Z'
jira: KT-22597
autotag-review: '2026-09-11T19:33:33.401Z'
TQID: 'https://experienceleague.adobe.com/p-OpyR7j7OiMGJbQCLITCniQaciNN3-Brby855pxVK8'
product_v2:
  - id: cb954087-f4fc-4456-afb9-e939cabcdc79
feature_v2:
  - id: 1210f62b-45d9-5d36-8d8d-0f9049ffa368
  - id: 2cb9b022-32b0-573b-a511-d091a4f355fb
  - id: df64005d-8f9a-422e-ba4d-c6f6dc3454b4
subfeature_v2:
  - id: d48edf2f-7bae-4df0-a9d4-7cabfb867d23
role_v2:
  - id: b69b2659-1057-424e-8fc5-ed9e016dc554
level_v2:
  - id: d378ca77-2da1-4f39-ad92-1917fe974a38
topic_v2:
  - id: e0eb8757-182f-49f3-94a4-1587d16f5094
source-git-commit: b812dd2c5228314771c572a184ae396aa400d524
workflow-type: tm+mt
source-wordcount: 76
ht-degree: 0%

---


# 사용자 지정 작업을 사용하여 과제 속성으로 콘텐츠 개인화

사용자 지정 작업으로 과제 데이터를 검색하고 이를 사용하여 각 구성원의 과제 진행 상황 및 보상에 따라 콘텐츠 카드 및 코드 기반 경험을 동적으로 개인화하는 방법에 대해 알아봅니다.

>[!VIDEO](https://video.tv.adobe.com/v/3503389/?captions=kor&learn=on&enablevpops)
