...
사용자가 참여하는 동안 모바일 앱 내에 인앱 메시지가 표시됩니다. 이러한 오버레이 스타일 메시지는 앱 인터페이스 바로 위에 배너, 팝업 또는 카드로 표시됩니다. 잠금 화면이나 앱 환경 외부에 나타나지 않습니다.

인앱 메시지는 특정 화면 보기, 구매 완료 또는 타겟팅된 대상 세그먼트에 대한 자격 부여와 같은 특정 사용자 작업 또는 조건에 의해 트리거됩니다.


해당 예는 다음과 같습니다.

* 사용자가 잠금을 해제하는 순간 새 기능을 소개하는 팝업을 표시하는 게임입니다.
* 쇼핑 앱은 사용자가 제품을 탐색하는 동안 쿠폰 코드가 있는 배너를 표시합니다.
* 현재 활동을 기반으로 팔로우할 새 계정을 제안하는 소셜 앱
